...
def sync_detailed(
    *,
    ...
    """Deletes one or multiple location object(s)

     Deletes one or multiple location objects based on the query filter attributes given as a parameters
    for the request. All
    location objects matching the given query are removed, thus caution should be exercised when using
    this operation. The
    return value reflects the success of the whole operation, thus either all location objects matching
    the query are removed
    or none is removed in case of an error.

    The success of the operation is based on the requested target state, e.g. if the deletion is
    requested based on UUID
    of a location object, the operation for that location object is successful also in a case where the
    given location does
    not exist (anymore).
