...
def sync_detailed(
    *,
    ...
    """Deletes one or multiple networks

     Deletes one or multiple networks based on the query filter attributes given as a parameters for the
    ...
    the success of the whole operation, thus either all networks matching the query are removed or none
    is removed in case of
    an error.

    The success of the operation is based on the requested target state, e.g. if a network deletion is
    requested based on UUID
    of the network, the operation for that network is successful also in a case where the given network
